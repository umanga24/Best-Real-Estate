...
class FrontEndController extends Controller
{
    ...
    public function homepage(Request $request)
    {
        ...
        // dd( $other_product );
        $properties = $this->product->orderBy('id', 'DESC')->limit(8)->get();
        ...
        return view('front.index', compact('sliders', 'other_product', 'properties', 'featured_category', 'meta_info', 'banner_category', 'all_post', 'recognizations', 'quotes'));
    }

    public function property()
    {
        $properties = $this->product->all();
       

        return view('front.propertylist.property', compact('properties'));
    }

    public function propertyDetails($slug, Request $request)
    {
        $property = $this->product->where(['slug'=>$slug])->first();
        //dd($property);
        return view('front.propertylist.propertydetails', compact('property'));
    }

    public function propertyBuy()
    {
        $propertyBuy = $this->product->where(['segment_id' => '2'])->get();

        return view('front.propertylist.buy', compact('propertyBuy'));
    }

    public function propertyRent()
    {
        $propertyRent = $this->product->where(['segment_id' => '1'])->get();

        return view('front.propertylist.rent', compact('propertyRent'));
    }
